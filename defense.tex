...
There is an entire function $L(E,s)$ called the \red{$L$-function} of $E$.  \\
...
RHS is the \red{analytic rank}, denoted by $r_{an}(E)$. \\
The BSD conjecture is open when $r_{an}(E) > 1$. \\

\medskip
\pause

The proof of BSD for $r_{an}(E) = 1$ uses a construction called \red{Heegner points}.
...
\item Rational functions on $X_0(N)$ are called {\bf modular functions}. They have {\bf q-expansions} at infinity:
\[
...
Let $r, u \in \bQ(C)$, a {\bf minimal polynomial relation} of $r$ and $u$ is an irreducible polynomial $P(x,y) \in \bQ[x,y]$, such that $P(r,u) = 0$.  \\
...
Let $\mathscr{E}_i(N)$ denote the set of elliptic points on $X_0(N)$ of period $i$, (i = 2,3).\\
